...
from app.services.dct_watermark import embed_dct, extract_dct, embed_dct_blind, extract_dct_blind
...
router = APIRouter(prefix="/dct", tags=["dct"])


@router.post("/embed")
async def embed_dct_api(
    ...
    try:
        ...
        output_path = os.path.join(temp_dir, f"dct_watermarked_{unique_id}.png")
        ...
        result, psnr_val = embed_dct(cover, watermark_img, alpha)
...
async def embed_dct_blind_api(
    ...
    try:
        validate_image_mime(image)
        if watermark: validate_image_mime(watermark)
        tmp = tempfile.gettempdir(); uid = str(uuid.uuid4())
        original_path  = os.path.join(tmp, f"orig_{uid}.png")
        watermark_path = os.path.join(tmp, f"wm_{uid}.png")
        output_path    = os.path.join(tmp, f"dct_blind_wm_{uid}.png")
        with open(original_path, "wb") as f: shutil.copyfileobj(image.file, f)
        validate_file_size(original_path); validate_magic_bytes(original_path)
        if watermark_type == "image":
            if watermark is None: raise HTTPException(status_code=400, detail="Watermark file required")
            with open(watermark_path, "wb") as f: shutil.copyfileobj(watermark.file, f)
            validate_file_size(watermark_path); validate_magic_bytes(watermark_path)
            watermark_img = cv2.imread(watermark_path, cv2.IMREAD_GRAYSCALE)
        elif watermark_type == "text":
            if not watermark_text: raise HTTPException(status_code=400, detail="Watermark text required")
            watermark_img = text_to_image(watermark_text); cv2.imwrite(watermark_path, watermark_img)
        else:
            raise HTTPException(status_code=400, detail="Invalid watermark_type")
        cover = cv2.imread(original_path)
        if cover is None or watermark_img is None: raise HTTPException(status_code=400, detail="Invalid image processing")
        result, psnr_val, (wm_h, wm_w) = embed_dct_blind(cover, watermark_img, alpha)
        cv2.imwrite(output_path, result)
        remove_file(original_path)
        if watermark_type == "image": remove_file(watermark_path)
        cleanup_task = BackgroundTask(remove_file, output_path)
        return FileResponse(output_path, media_type="image/png", filename="watermarked_blind.png",
            headers={"X-PSNR": f"{psnr_val:.2f}", "X-Alpha": f"{alpha:.4f}",
                     "X-Method": "DCT-Blind", "X-WM-Height": str(wm_h), "X-WM-Width": str(wm_w)},
            background=cleanup_task)
    except Exception as e:
        for p in (original_path, watermark_path, output_path):
...
async def extract_dct_blind_api(
    background_tasks: BackgroundTasks,
    ...
    try:
        ...
        tmp = tempfile.gettempdir(); uid = str(uuid.uuid4())
        wm_path       = os.path.join(tmp, f"wm_{uid}.png")
        input_wm_path = os.path.join(tmp, f"input_wm_{uid}.png")
        output_path   = os.path.join(tmp, f"dct_blind_ext_{uid}.png")
        with open(wm_path, "wb") as f: shutil.copyfileobj(watermarked.file, f)
        ...
        if watermark_type == "image" and watermark_input:
            ...
            with open(input_wm_path, "wb") as f: shutil.copyfileobj(watermark_input.file, f)
            has_ref = True
        elif watermark_type == "text" and watermark_text:
            cv2.imwrite(input_wm_path, text_to_image(watermark_text)); has_ref = True
        wm_img = cv2.imread(wm_path)
        if wm_img is None: raise HTTPException(status_code=400, detail="Cannot read watermarked image")
        wm_size   = (wm_height, wm_width) if (wm_height and wm_width) else None
        extracted = extract_dct_blind(wm_img, alpha, watermark_size=wm_size)
...
async def extract_dct_api(
    ...
    try:
        ...
        output_path = os.path.join(temp_dir, f"dct_extracted_{unique_id}.png")
        ...
        # Determine target output size from reference watermark if available
        watermark_size = None
        if has_reference_wm:
            ref_for_size = cv2.imread(input_wm_path, cv2.IMREAD_GRAYSCALE)
            if ref_for_size is not None:
                watermark_size = (ref_for_size.shape[0], ref_for_size.shape[1])

        extracted = extract_dct(orig_img, wm_img, alpha, watermark_size=watermark_size)
        cv2.imwrite(output_path, extracted)
        ...
        if has_reference_wm:
            ...
            # extracted is already resized to original_watermark dimensions
            nc_val = normalized_correlation(original_watermark, extracted)
