...
from app.services.dft_watermark import embed_dft, extract_dft, embed_dft_blind, extract_dft_blind
...
async def embed_dft_api(
    ...
    try:
        ...
        output_path = os.path.join(temp_dir, f"dft_watermarked_{unique_id}.png")
        ...
        result, psnr_val = embed_dft(cover, watermark_img, alpha)
...
async def embed_dft_blind_api(
    ...
    try:
        ...
        output_path    = os.path.join(tmp, f"dft_blind_wm_{uid}.png")
        ...
        result, psnr_val, (wm_h, wm_w) = embed_dft_blind(cover, watermark_img, alpha)
        cv2.imwrite(output_path, result)
        remove_file(original_path)
        ...
        return FileResponse(output_path, media_type="image/png", filename="watermarked_blind.png",
            headers={"X-PSNR": f"{psnr_val:.2f}", "X-Alpha": f"{alpha:.4f}",
                     "X-Method": "DFT-Blind", "X-WM-Height": str(wm_h), "X-WM-Width": str(wm_w)},
            background=cleanup_task)
    except Exception as e:
        for p in (original_path, watermark_path, output_path):
...
async def extract_dft_blind_api(
    background_tasks: BackgroundTasks,
    ...
    try:
        ...
        output_path   = os.path.join(tmp, f"dft_blind_ext_{uid}.png")
        ...
        extracted = extract_dft_blind(wm_img, alpha, watermark_size=wm_size)
...
async def extract_dft_api(
    ...
    try:
        ...
        output_path = os.path.join(temp_dir, f"dft_extracted_{unique_id}.png")
        ...
        # Determine watermark output size from reference if available
        watermark_size = None
        if has_reference_wm:
        ...
        extracted = extract_dft(orig_img, wm_img, alpha, watermark_size=watermark_size)
        cv2.imwrite(output_path, extracted)
